...
export class Game extends BaseDemo {
	...
	public constructor() {
		...
		//
		this.createText("Press W/A/S/D to move.\nPress Q/E/SPACE to switch weapons and skin.\nTouch to aim and fire.");
		//
		const factory = dragonBones.PixiFactory.factory;
		factory.parseDragonBonesData(Loader.shared.resources["dragon/mecha_1502b/mecha_1502b_ske.json"].data);
		factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/mecha_1502b/mecha_1502b_tex.json"].data,
			Loader.shared.resources["dragon/mecha_1502b/mecha_1502b_tex.png"].texture
		);
		factory.parseDragonBonesData(Loader.shared.resources["dragon/skin_1502b/skin_1502b_ske.json"].data);
		factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/skin_1502b/skin_1502b_tex.json"].data,
			Loader.shared.resources["dragon/skin_1502b/skin_1502b_tex.png"].texture
		);
		factory.parseDragonBonesData(Loader.shared.resources["dragon/weapon_1000/weapon_1000_ske.json"].data);
		factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/weapon_1000/weapon_1000_tex.json"].data,
			Loader.shared.resources["dragon/weapon_1000/weapon_1000_tex.png"].texture
		);
		//
	...
	private _enterFrameHandler(_deltaTime: number): void {
		// Make sure game update before dragonBones update.
		if (this._player) {
			this._player.update();
		}
...
class Mecha {
	...
	private readonly _target: PointType = new Point();
	private readonly _helpPoint: PointType = new Point();
...
class Bullet {
	...
	public update(): boolean {
		...
		if (
			this._armatureDisplay.x < -Game.instance.stageWidth * 0.5 - 100.0 ||
			this._armatureDisplay.x > Game.instance.stageWidth * 0.5 + 100.0 ||
			this._armatureDisplay.y < -Game.instance.stageHeight * 0.5 - 100.0 ||
			this._armatureDisplay.y > Game.instance.stageHeight * 0.5 + 100.0
		) {
