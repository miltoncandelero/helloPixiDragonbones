...
import { Point, Ticker, Loader } from "pixi.js";
import dragonBones from "../lib/dragonBones";
import { DragHelper } from "./DragHelper";

export class BoundingBox extends BaseDemo {
	private readonly _helpPointA: Point = new Point();
	private readonly _helpPointB: Point = new Point();
	private readonly _helpPointC: Point = new Point();
	...
	public constructor() {
		...
		factory.parseDragonBonesData(Loader.shared.resources["dragon/mecha_2903/mecha_2903_ske.json"].data);
		factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/mecha_2903/mecha_2903_tex.json"].data,
			Loader.shared.resources["dragon/mecha_2903/mecha_2903_tex.png"].texture
		);
		factory.parseDragonBonesData(Loader.shared.resources["dragon/bounding_box_tester/bounding_box_tester_ske.json"].data);
		factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/bounding_box_tester/bounding_box_tester_tex.json"].data,
			Loader.shared.resources["dragon/bounding_box_tester/bounding_box_tester_tex.png"].texture
		);
		//
	...
	private _enterFrameHandler(_deltaTime: number): void {
		...
		const intersectsSlots = this._armatureDisplay.armature.intersectsSegment(
			this._helpPointA.x,
			this._helpPointA.y,
			this._helpPointB.x,
			this._helpPointB.y,
	...
	destroy(options?: { children?: boolean; texture?: boolean; baseTexture?: boolean }): void {
		...
		DragHelper.getInstance().disableDrag(this._targetA);
		DragHelper.getInstance().disableDrag(this._targetB);
