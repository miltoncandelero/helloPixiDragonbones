...
import { Loader, interaction, Text } from "pixi.js";

export class ReplaceSlotDisplay extends BaseDemo {
	...
	private _logoText: Text;

	public constructor() {
		super();

		this._factory.parseDragonBonesData(Loader.shared.resources["dragon/mecha_1004d_show/mecha_1004d_show_ske.json"].data);
		this._factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/mecha_1004d_show/mecha_1004d_show_tex.json"].data,
			Loader.shared.resources["dragon/mecha_1004d_show/mecha_1004d_show_tex.png"].texture
		);
		this._factory.parseDragonBonesData(Loader.shared.resources["dragon/weapon_1004_show/weapon_1004_show_ske.json"].data);
		this._factory.parseTextureAtlasData(
			Loader.shared.resources["dragon/weapon_1004_show/weapon_1004_show_tex.json"].data,
			Loader.shared.resources["dragon/weapon_1004_show/weapon_1004_show_tex.png"].texture
		);
		//
		...
		const touchHandler = (event: interaction.InteractionEvent) => {
	...
	private _replaceDisplay(type: number): void {
		if (type === -1) {
			...
			if (logoSlot.display === this._logoText) {
				...
				if (!this._logoText) {
					this._logoText = new Text("Core Element");
